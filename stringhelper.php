...
function __phone_tokenize($value)
{
    ...
    foreach (__phone_area_codes() as $area_codes__value) {
...
{
    $return = [];
    $value = __phone_tokenize($value);
    if (__x($value['country_code'])) {
        $return[] = '+' . $value['country_code'];
    }
    if (__x($value['area_code'])) {
...
{
    return array_merge(__phone_area_codes_landline(), __phone_area_codes_mobile());
}
...
{
    $value = __phone_tokenize($value);
    if (in_array($value['area_code'], __phone_area_codes_landline())) {
        return true;
    }
    return false;
}
...
{
    $value = __phone_tokenize($value);
    if (in_array($value['area_code'], __phone_area_codes_mobile())) {
        return true;
    }
    return false;
}
...
function __curl($url = '', $data = null, $method = null, $headers = null, $enable_cookies = false, $send_as_json = true, $timeout = 60, $basic_auth = null)
{
    ...
    if (__x($basic_auth)) {
        curl_setopt($curl, CURLOPT_USERPWD, array_key_first($basic_auth) . ':' . $basic_auth[array_key_first($basic_auth)]);
    }
